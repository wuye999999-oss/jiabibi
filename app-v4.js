...
const official=['官方旗舰店','品牌旗舰店','旗舰店','官方店','京东自营','自营','官方自营'];
const channel=['渠道','批发','厂家','工厂','源头','尾货','清仓','仓库','量贩','整箱批发','团购','商用','大包装','批发价','囤货','囤装'];
const cats={
  yogurt:['酸奶','低温酸奶','常温酸奶','风味发酵乳','发酵乳','乳酸菌','酸乳'],
  milk:['牛奶','纯牛奶','鲜牛奶','牛乳','蛋白牛乳'],
  water:['百岁山','农夫山泉','怡宝','矿泉水','纯净水','天然水','饮用水','矿物质水'],
  cola:['可口可乐','coca','cola','雪碧','芬达','百事','汽水','碳酸','可乐'],
  paper:['纸巾','抽纸','卷纸','面巾纸','卫生纸','卷筒纸'],
  laundry:['洗衣液','洗衣凝珠','洗衣粉'],
  power:['充电宝','移动电源','毫安','mah','快充'],
  case:['手机壳','保护壳','硅胶壳','透明壳']
};
const brands={xiaomi:['小米','xiaomi','redmi','米家'],anker:['安克','anker'],ganten:['百岁山','ganten'],nongfu:['农夫山泉'],cestbon:['怡宝'],coke:['可口可乐','coca-cola','cocacola','coca'],sprite:['雪碧','sprite'],fanta:['芬达','fanta'],pepsi:['百事','pepsi'],vinda:['维达','vinda'],blue:['蓝月亮'],zhenling:['真零'],apple:['苹果','apple','iphone'],huawei:['华为','huawei']};
...
const hardEx=['刻字','激光','礼物','礼盒','摆件','挂件','钥匙扣','贴纸','海报','模型','手办','空瓶','收藏','周边','定制','抱枕','衣服','杯子','水杯','开瓶器','配件','适用','兼容','支架','贴膜','数据线'];
...
const imageOf=i=>i.goods_thumbnail_url||i.goods_image_url||i.image||'';
...
function intent(q){
  ...
  if(cat==='water' && ['可乐','雪碧','芬达','汽水','碳酸','cola'].some(x=>n.includes(norm(x)))) cat='cola';
...
function parseSpec(i,q=''){
  ...
  m=t.match(/(\d+(?:\.\d+)?)\s*(kg|KG|斤|g|G)\s*[*]\s*(\d+)\s*(桶|瓶|袋|盒|包|杯)?/);
  if(m){let kg=toKg(Number(m[1]),m[2])*Number(m[3]); return {kind:'weight',text:`${yuan(kg)}kg ｜ 约 ¥${yuan(p/kg)}/kg`,value:p/kg};}
  m=t.match(/(\d+)\s*(桶|瓶|袋|盒|包|杯).{0,12}?(\d+(?:\.\d+)?)\s*(kg|KG|斤|g|G)/);
  if(m){let kg=toKg(Number(m[3]),m[4])*Number(m[1]); return {kind:'weight',text:`${yuan(kg)}kg ｜ 约 ¥${yuan(p/kg)}/kg`,value:p/kg};}
  m=t.match(/(\d+(?:\.\d+)?)\s*(kg|KG|斤|g|G)/);
  if(m){let kg=toKg(Number(m[1]),m[2]); return {kind:'weight',text:`${yuan(kg)}kg ｜ 约 ¥${yuan(p/kg)}/kg`,value:p/kg};}
...
function score(i,q){
  const it=intent(q), t=tx(i); let s=0;
  for(const b of it.brands){ if(has(i.goods_name||'',brands[b])) s+=500; else if(has(t,brands[b])) s+=160; }
  if(it.cat&&has(i.goods_name||'',cats[it.cat])) s+=260; else if(it.cat&&has(t,cats[it.cat])) s+=80;
  for(const a of it.attrs){ if(has(i.goods_name||'',attrs[a])) s+=260; else if(has(t,attrs[a])) s+=120; }
  if(String(i.source||'').includes('exact_input')||String(i.source||'').includes('scrape')) s+=1000;
  return s;
}
...
function best(list,type,p,q){return list.filter(x=>x.platform===p&&typeOf(x)===type&&relevant(x,q)).sort((a,b)=>valueOf(a,q)-valueOf(b,q)||score(b,q)-score(a,q)||priceOf(a)-priceOf(b))[0];}
function bestAll(list,type,q){return platforms.map(([p])=>best(list,type,p,q)).filter(Boolean).sort((a,b)=>valueOf(a,q)-valueOf(b,q)||score(b,q)-score(a,q)||priceOf(a)-priceOf(b))[0];}
async function loadProviderStatus(){try{const r=await fetch(API+'/api/providers/status'); const d=await r.json(); providerStatus=d.providers||[];}catch(e){providerStatus=[];}}
...
function renderProviders(data={}){
  ...
  for(const [p,name] of platforms){const meta=providerMeta(p), st=(lastSearchProviders||[]).find(x=>x.platform===p); let cls=meta.configured?'ok':'warn', txt=meta.configured?'已配置':'待接入'; if(meta.search===false) txt='待接入'; if(st){ if(st.error){cls='bad';txt='失败'} else if(st.ok&&st.source!=='provider_placeholder'){cls='ok';txt=(st.source||'真实接口')+' · '+(st.total_count||0)} else if(st.message) txt=st.message;} box.insertAdjacentHTML('beforeend',`<div class="box"><b>${name}</b><div class="pill ${cls}">${txt}</div><div class="empty">${meta.source||''}</div></div>`);}
}
function buyButton(item){return item?`<button class="buy" type="button" onclick="buyByKey('${item._key}')">去购买</button>`:'';}
function summaryCard(data,q){const all=data.goods_list||[], kept=all.filter(x=>relevant(x,q)); return `<div class="card muted">候选 ${all.length} 条；保留 ${kept.length} 条；过滤 ${all.length-kept.length} 条。当前意图：${JSON.stringify(intent(q))}</div>`;}
function itemMainHtml(item,q){const img=imageOf(item); const pf=platforms.find(x=>x[0]===item.platform)?.[1]||item.platform; const sp=spec(item,q); return `${img?`<img src="${img}" style="width:64px;height:64px;object-fit:cover;border-radius:16px;float:left;margin-right:12px">`:''}<div class="price">¥${yuan(priceOf(item))}</div><div class="muted">${pf} · ${item.shop_name||item.brand_name||''}<br>${item.goods_name||''}</div>${sp?`<div class="spec">${sp}</div>`:''}`;}
function renderResult(data,q){renderProviders(data); const list=(data.goods_list||[]).filter(x=>relevant(x,q)); const r=$('panel-result'); r.innerHTML=summaryCard(data,q); if(!list.length){r.insertAdjacentHTML('beforeend','<div class="card muted">没有可靠匹配。为了避免错品，已过滤掉不满足品牌、品类、强属性或规格的结果。</div>');return;} for(const [type,title] of groups){const item=bestAll(list,type,q); let html='<div class="card"><div class="row"><div><div class="muted">'+title+'</div>'; html+=item?itemMainHtml(item,q):'<div class="price">¥--</div><div class="muted">未返回该类型商品。</div>'; html+='</div>'+buyButton(item)+'</div><div class="grid">'; for(const [p,name] of platforms){const x=best(list,type,p,q); html+='<div class="p"><div class="muted">'+name+'</div>'+(x?'<b>¥'+yuan(priceOf(x))+'</b><div class="empty">'+(spec(x,q).split('｜').pop()||'')+'</div>':'<div class="empty">待接入/暂无</div>')+'</div>'; } html+='</div></div>'; r.insertAdjacentHTML('beforeend',html);}}
function renderDebug(data,q){const rows=(data.goods_list||[]).map(i=>({i,d:diagnose(i,q)})); const keep=rows.filter(x=>x.d.keep).length; const box=$('panel-debug'); box.innerHTML=`<div class="card muted">接口候选 ${rows.length} 条；保留 ${keep} 条；过滤 ${rows.length-keep} 条。<br>当前意图：${JSON.stringify(intent(q))}</div>`; rows.forEach((r,idx)=>{const i=r.i,d=r.d,pf=(platforms.find(x=>x[0]===i.platform)||[])[1]||i.platform; box.insertAdjacentHTML('beforeend',`<div class="card candidate"><div class="row"><div><span class="pill ${d.keep?'ok':'bad'}">${d.keep?'保留':'过滤'}</span><span class="pill">#${idx+1}</span><span class="pill">${pf}</span><span class="pill">${i.source||''}</span></div><div>${buyButton(i)}</div></div>${itemMainHtml(i,q)}<div class="reason">${d.reasons.join('；')}</div></div>`);});}
